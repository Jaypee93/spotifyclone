...
const SongRow = ({ images, title, artist, album, duration, i, loading, position, contextUri, spotifyApi }) => {
	...
	const onRowClick = async () => {
		const song = { context_uri: contextUri, offset: { position }, position_ms: 0 };
	...
	return (
		...
		>
			<Grid sx={{ width: 35, display: 'flex', alignItems: 'center', fontSize: 15 }} item>
				{i + 1}
			</Grid>
			...
			<Grid xs={3} item sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center' }}>
				{loading ? <Skeleton variant="text" width={50} height={18} /> : album}
			</Grid>
